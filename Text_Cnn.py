...
class ConvNet(nn.Module):# inherit from nn.Module
    ...
    def __init__(self):#init the module
        ...
        #Fc layer
        self.fc1 = nn.Linear(FILTER_NUM[0]+FILTER_NUM[1]+FILTER_NUM[2], 80) 
    ...
    def single_instance_forward(self,x):
    ...
    def get_gradient(self,x,y,loss_func=nn.NLLLoss()):
        '''
        get the gradient of Embedding layer for a given x
        '''
        # get Embedding of x
        embed = self.embed(x)
        embed.requires_grad_()
        # then forward propagation
        out1 = self.conv1(embed) 
        out2 = self.conv2(embed)
        out3 = self.conv3(embed)
        out1 = torch.tanh(out1)
        out2 = torch.tanh(out2)
        out3 = torch.tanh(out3)
        max1,_ = torch.max(out1,2)
        ...
        Out = torch.tanh(Out)
        Out = self.fc3(Out) # batch*20 -> batch*2
        y_hat = f.log_softmax(Out,dim=1) #"dim=1" means logsoftmax along 2 sentiment(pos or neg)
        # then comput loss
    ...
    def predict(self,x):
        Out = self.single_instance_forward(x)
        if Out[0][0]> Out[0][1]:
            y_hat = 'POS'
        else:
            y_hat = 'NEG'
...
def class_to_tensor(class_name):
    if class_name[0] == "NEG":
